...
export default {
	...
	methods: {
		m_change_image() {
			...
			span_order[1].n = this.d_current_image + 1 >= this.d_images.length ? 0 : this.d_current_image + 1;
...
<template>
	<div class="landing_carousel">
		<div class="inner">
			<span class="image" :style="{zIndex: d_image_flipflop ? 2 : 1, backgroundImage: `url(${d_images[d_image_1.n]})`, opacity: d_image_1.opacity}"></span>
			<span class="image" :style="{zIndex: d_image_flipflop ? 1 : 2, backgroundImage: `url(${d_images[d_image_2.n]})`, opacity: d_image_2.opacity}"></span>
		</div>
	</div>
</template>
